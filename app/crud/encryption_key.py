...
def get_encryption_key_by_user_id(session: Session, *, user_id: UUID) -> Optional[EncryptionKey]:
    statement = select(EncryptionKey).where(EncryptionKey.user_id == user_id)
    return session.exec(statement).first()


def create_encryption_key(session: Session, *, user_id: UUID, wrapped_key: str) -> EncryptionKey:
